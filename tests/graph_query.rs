...
fn kql_filters_nodes_by_type() {
    ...
    // Output should include created_at timestamps in " @ YYYY-MM..." format
    assert!(output.contains(" @ 2026-"));
}

#[test]
fn kql_supports_sort_by_created_at_asc() {
    let dir = temp_workspace();
    write_fixture(&test_graph_root(dir.path()));
    let output = exec_ok(
        &["kg", "fridge", "kql", "node type=Concept sort=created_at"],
        dir.path(),
    );
    let lines: Vec<&str> = output.lines().collect();
    // Find lines with @ timestamps (skip header lines)
    ...
    assert!(
        timestamps.len() >= 2,
        "should have at least 2 nodes with timestamps, got: {timestamps:?}"
    );
    // Ascending order: first timestamp should be <= last
    let first_ts = timestamps[0].split(" @ ").nth(1).unwrap();
    let last_ts = timestamps[timestamps.len() - 1].split(" @ ").nth(1).unwrap();
    assert!(
        first_ts <= last_ts,
        "ascending sort failed: first={first_ts}, last={last_ts}"
    );
}

#[test]
fn kql_supports_sort_by_created_at_desc() {
    let dir = temp_workspace();
    write_fixture(&test_graph_root(dir.path()));
    let output = exec_ok(
        &["kg", "fridge", "kql", "node sort=-created_at limit=5"],
        dir.path(),
    );
    let lines: Vec<&str> = output.lines().collect();
    let timestamps: Vec<&str> = lines
    ...
    assert!(
        timestamps.len() >= 2,
        "should have at least 2 nodes with timestamps"
    );
    // Descending order: first timestamp should be >= last
    let first_ts = timestamps[0].split(" @ ").nth(1).unwrap();
    let last_ts = timestamps[timestamps.len() - 1].split(" @ ").nth(1).unwrap();
    assert!(
        first_ts >= last_ts,
        "descending sort failed: first={first_ts}, last={last_ts}"
    );
}

#[test]
fn kql_supports_sort_by_importance_desc() {
    let dir = temp_workspace();
    write_fixture(&test_graph_root(dir.path()));
    let output = exec_ok(
        &["kg", "fridge", "kql", "node type=Feature sort=-importance limit=10"],
        dir.path(),
    );
    assert!(output.contains("nodes:"));
    // Multiple Feature nodes should be sorted by importance desc
    let lines: Vec<&str> = output.lines().collect();
    let feature_lines: Vec<&str> = lines
        .iter()
        .filter(|l| l.contains(" # ") && l.contains(" [Feature]"))
        .copied()
        .collect();
    // If we have multiple features, they should be in descending importance order
    // (we can't easily parse importance from CLI output, but we verify sort didn't break)
    assert!(!feature_lines.is_empty(), "should return Feature nodes");
}

#[test]
fn kql_sort_limit_combined() {
    let dir = temp_workspace();
    write_fixture(&test_graph_root(dir.path()));
    let output = exec_ok(
        &["kg", "fridge", "kql", "node sort=-created_at limit=3"],
        dir.path(),
    );
    ...
    assert!(
        node_lines.len() <= 3,
        "limit should be respected: got {} nodes",
        node_lines.len()
    );
...
#[test]
fn kql_sort_unknown_field_graceful() {
    let dir = temp_workspace();
    write_fixture(&test_graph_root(dir.path()));
    // Unknown sort field should not panic, just skip sorting
    let output = exec_ok(
        &["kg", "fridge", "kql", "node sort=unknown_field"],
        dir.path(),
    );
    assert!(output.contains("nodes:"));
    // Should still return results, just without custom sort
}
...
#[test]
fn kql_query_returns_created_at_in_output() {
    let dir = temp_workspace();
    write_fixture(&test_graph_root(dir.path()));
    let output = exec_ok(
        &["kg", "fridge", "kql", "node sort=-created_at limit=3"],
    ...
    // Output should include timestamps in " @ YYYY-MM..." format
    let lines: Vec<&str> = output.lines().collect();
    let with_timestamps = lines.iter().filter(|l| l.contains(" @ 2026-")).count();
    assert!(
        with_timestamps >= 2,
        "should include created_at timestamps in output"
    );
}

#[test]
fn kql_filter_by_valid_to_empty_finds_currently_valid() {
    let dir = temp_workspace();
    write_fixture(&test_graph_root(dir.path()));
    // Add a node with validity period
    exec_ok(
        &[
            "kg", "fridge", "node", "add",
            "concept:test_valid",
            "--type", "Concept",
            "--name", "Test Valid",
            "--description", "Test node with validity",
            "--provenance", "U",
            "--confidence", "0.9",
    ...
    // Should include (valid_from: ...) if validity fields are shown
    assert!(output.contains("# concept:") || output.contains("nodes:"));
}

#[test]
fn kg_node_find_skip_feedback_flag() {
    let dir = temp_workspace();
    write_fixture(&test_graph_root(dir.path()));
    // Should accept --skip-feedback flag (previously caused "unknown option" error)
    let output = exec_ok(
        &["kg", "fridge", "node", "find", "fridge", "--skip-feedback"],
        dir.path(),
    );
    // Should find results without prompting for feedback
    assert!(output.contains("fridge") || output.contains("nodes"));
}

#[test]
fn kg_graph_list_command() {
    let dir = temp_workspace();
    write_fixture(&test_graph_root(dir.path()));
    // List command should work via KQL
    let output = exec_ok(
        &["kg", "fridge", "list", "--limit", "5"],
        dir.path(),
    );
    // Should show nodes with timestamps
    assert!(output.contains("nodes:") || output.contains("# "));
}

#[test]
fn kg_node_add_with_valid_to() {
    let dir = temp_workspace();
    write_fixture(&test_graph_root(dir.path()));
    // Add a node with validity period
    let output = exec_ok(
        &[
            "kg", "fridge", "node", "add",
            "bug:test_valid_period",
            "--type", "Bug",
            "--name", "Test Bug",
            "--description", "Bug with validity period",
            "--provenance", "U",
            "--confidence", "0.9",
            "--valid-to", "2026-04-30",
        ],
...
#[test]
fn kg_node_modify_with_valid_to() {
    let dir = temp_workspace();
    write_fixture(&test_graph_root(dir.path()));
    // Add a node first
    exec_ok(
    ...
    // Modify with valid_to
    let output = exec_ok(
        &[
            "kg", "fridge", "node", "modify",
            "bug:modify_me",
            "--valid-to", "2026-05-01",
        ],
        dir.path(),
    );
    // Should show diff with valid_to change
    assert!(output.contains("valid_to") || output.contains("~ node"));
}
